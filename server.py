...
def serve_local_audio(file_path):
    ...
    try:
        ...
        default_audio_path = os.path.join(os.path.dirname(DEFAULT_AUDIO_DIR), file_path)
        ...
        # 安全检查，防止路径遍历
        if not (actual_path.startswith(LOCAL_STORAGE_DIR) or actual_path.startswith(os.path.dirname(DEFAULT_AUDIO_DIR))):
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 使用0.0.0.0确保服务器可以从任何网络接口访问
        logger.info(f"Server starting on http://{HOST}:{PORT}, debug mode: {DEBUG}")
